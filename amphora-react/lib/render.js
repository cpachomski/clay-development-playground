...
function makeHtml(state) {
  return function(_data) {
    ...
    console.log(getValidReactComponents(_data));
...
function getValidReactComponents(data) {
  ...
  for (let k in data) {
    const components = data[k];
    components
      .map(c => {
        const componentName = clayUtils.getComponentName(c._ref);
        return getComponentPath(componentName);
      })
      .filter(path => {
        try {
          ...
          if (component) {
            ...
            validReactComponents.push(component);
          }
        } catch (error) {
          log("error", `Cannot find valid React module for path: ${path}`, {
            code: error.code || "N/A"
          });
        }
      });
...
function makeState(data, meta) {
  ...
  if (meta._layoutRef) {
    ...
    if (meta.locals.edit) {
      initialState._layoutData.schema = getSchemaPath(
  ...
  initialState._componentVariations = styleguide.getVariations(
...
module.exports.setHbs = val => {
  hbs = val;
};

// For testing
module.exports.setLog = fakeLog => {
  log = fakeLog;
};
module.exports.makeState = makeState;
module.exports.setFsFns = ({
  getIndicesStub,
  getSchemaPathStub,
  getComponentPathStub
}) => {
